...
use App\Models\Cms\MembershipModel;
use App\Models\Cms\SuperModel;


class Membership extends Super
{
  ...
  public function membership()
    ...
    return redirect()->to(base_url('Cms/Login/index'))->send(); // Ensure correct URL and use base_ur
        ...
        $data = array();
        $data['db_table_name'] = "membership";
        $data['record_name']   = "membership";
        $data['records_name'] = $data['menu_active'] = "membership";
        $data['form_link'] = site_url("cms/membership/membership_form/");
        $data['table_link'] = site_url("Cms/membership/membership");
        ...
        $data['records'] = $this->MembershipModel->selectmembership_model_m();
    ...
    public function membership_form($id = 0)
    {
        $data = array();
        $data['db_table_name'] = "membership";
        $data['record_name']   = "membership";
        $data['records_name'] = $data['menu_active'] = "membership";
        $data['table_link'] = site_url("Cms/membership/membership");
        $data['submit_link'] = site_url("Cms/membership/membership_save");
        $data['id'] = $id;
        ...
        return $this->render("membership_form", $data);
    }
    public function membership_save()
    {
        ...
        if (!empty($photo)) {
            ...
            $membership = $this->SuperModel->selectSingleData_m($tableName, $id);

            $uploadImage = $membership['image'];
        }
        ...
        if (!$id > 0) {
            ...
            $this->swalSuccess(site_url("Cms/membership/membership"));
        }
        die();
        return  redirect()->to('Cms/membership/membership');
    }
