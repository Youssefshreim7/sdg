...
use App\Models\Cms\SliderModel;
...
class Slider extends Super
{
  ...
  public function sliders()
    ...
    return redirect()->to(base_url('Cms/Login/index'))->send(); // Ensure correct URL and use base_ur
        ...
        $data = array();
        $data['db_table_name'] = "sliders";
        $data['record_name']   = "Slider";
        $data['records_name'] = $data['menu_active'] = "Sliders";
        $data['form_link'] = site_url("cms/slider/slider_form/");
        $data['table_link'] = site_url("Cms/Slider/sliders");
        $data['table_icon'] = '<i class="kt-menu__link-icon  flaticon-confetti"></i>';
       

        $data['th'] = ["ID", "Name", "Position", "image", "date_created", "status", "action"];
        $data['columns'] = ["id", "name", "position", "image", "date_created", "is_blocked", "is_active"];
        $data['records'] = $this->SliderModel->selectSliders_model_m();
    ...
    public function slider_form($id = 0)
    {
        $data = array();
        $data['db_table_name'] = "sliders";
        $data['record_name']   = "Slider";
        $data['records_name'] = $data['menu_active'] = "Sliders";
        $data['table_link'] = site_url("Cms/Slider/sliders");
        $data['submit_link'] = site_url("Cms/Slider/slider_save");
        $data['id'] = $id;
        ...
        return $this->render("slider_form", $data);
    }
    public function slider_save()
    {
        ...
        if (!empty($photo)) {
            ...
            $slider = $this->SuperModel->selectSingleData_m($tableName, $id);
        ...
        $dbData = array(
            ...
            "btn_action_link" => $data['btn_action_link'],
        ...
        if (!$id > 0) {
            ...
            $this->swalSuccess(site_url("Cms/Slider/sliders"));
        }
        die();
        return  redirect()->to('Cms/Slider/sliders');
    }
