...
class Team extends Super
{
  ...
  public function team()
    ...
    return redirect()->to(base_url('Cms/Login/index'))->send(); // Ensure correct URL and use base_ur
        ...
        $data = array();
        $data['db_table_name'] = "team";
        $data['record_name']   = "team";
        $data['records_name'] = $data['menu_active'] = "team";
        $data['form_link'] = site_url("cms/team/team_form/");
        $data['table_link'] = site_url("Cms/team/team");
        ...
        $data['records'] = $this->TeamModel->selectteam_model_m();
    ...
    public function team_form($id = 0)
    {
        $data = array();
        $data['db_table_name'] = "team";
        $data['record_name']   = "team";
        $data['records_name'] = $data['menu_active'] = "team";
        $data['table_link'] = site_url("Cms/team/team");
        $data['submit_link'] = site_url("Cms/team/team_save");
        $data['id'] = $id;
        ...
        return $this->render("team_form", $data);
    }
    public function team_save()
    {
        ...
        if (!empty($photo)) {
            ...
            $team = $this->SuperModel->selectSingleData_m($tableName, $id);
        ...
        if (!$id > 0) {
            ...
            $this->swalSuccess(site_url("Cms/team/team"));
        }
        die();
        return  redirect()->to('Cms/team/team');
    }
