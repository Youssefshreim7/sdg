					...
					<div class="card">
						<div class="card-body">
							<div class="row">
								<div class="col-lg-6">
									<div class="mb-3">
										<label class="form-label" for="project-title-input">Sub Category Title</label>
										<input type="text" name="name" class="form-control" placeholder="Enter Sub Category Title" value="<?= @$record['name'] ?>">
									</div>
								</div>
								<div class="col-lg-6 mb-3 ">
									<label class="form-label">Category </label>
									<select class="form-control js-example-basic-single" data-placeholder="Select Category" name="cat_id" required>
										<option></option>
										<?php foreach ($categories as $key => $value) {   ?>
											<option value="<?= $value['id']; ?>"
												<?= (isset($record['cat_id'])  && $record['cat_id'] == $value['id']) ? 'selected' : ''; ?>><?= $value['name']; ?>
											</option>
										<?php } ?>
									</select>
								</div>
